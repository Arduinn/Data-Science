...
for name in resultData.keys():
    resultData[name].iloc[:, 1:] = resultData[name].iloc[:, 1:].round(3)
    ...
    resultData[name].to_csv(f"{file_path}_commadecimal.csv", sep=";", decimal=",", index=True)  # Set index=True if you want to include the index